...
namespace UnityEditorInternal.VersionControl
{
	...
	public class ListControl
	{
		...
		const string c_changeKeyPrefix = "_chkeyprfx_"; // Hack to make path mapping refresh work with changesets
		...
		// would not need this.  This is not thread safe and feels like a nasty hack.
		...
		public ListControl ()
		{
			// Assign a unique id.  Bit of a hack to pass the class as an int
		...
		public void Sync ()
		{
			...
			foreach (UnityEngine.Object obj in Selection.objects)
			{
				if (AssetDatabase.IsMainAsset (obj))
				{
					...
					string path = projectPath + AssetDatabase.GetAssetOrScenePath (obj);
		...
		private bool HandleMouse(Rect area)
		{
			...
			else if (e.type == EventType.MouseUp && mouseInArea)
			{
				GUIUtility.keyboardControl = 0;
				singleSelect = GetItemAt(area, e.mousePosition);
				dragCount = 0;
				...
				if (singleSelect != null && !singleSelect.Dummy)
				{
					...
					if (e.button == 1)
					{
						...
						if (IsSelectedFolder() && !string.IsNullOrEmpty(menuFolder))
						{
					...
					else if (e.button == 0 && !(e.control || e.command || e.shift))
					{
						if (IsSelected(singleSelect))
						{
							SelectedSet(singleSelect);
		...
		void DrawItem (ListItem item, Rect area, float x, float y, bool focus, bool selected)
		{
			...
			if (highlight)
			{
				...
				GUI.Label (new Rect (x + 32, y, area.width - (x + 32), c_lineHeight + 2), DisplayName (item));
			}
			else
			{
				GUI.Label (new Rect (x + 32, y, area.width - (x + 32), c_lineHeight + 2), DisplayName (item));
		...
		// Is a folder selected in the list
		bool IsSelectedFolder ()
		{
			foreach (KeyValuePair<string, ListItem> de in selectList)
			{
				if (de.Value != null && de.Value.Asset != null && !de.Value.Asset.isFolder)
				{
					return false;
				}
			}
		...
		ListItem GetTwin(ListItem item)
		{
			...
			if (item.Next != null && item.Next.Asset.path.ToLower() == (name.TrimEnd('/') + ".meta"))
				return item.Next;
			else if (item.Prev != null && (item.Prev.Asset.path.ToLower().TrimEnd('/') + ".meta") == name)
				return item.Prev;
			return null;
		}
		...
		public void SelectedAdd (ListItem item)
		{
			...
			name = name.EndsWith(".meta") ? name.Substring(0, name.Length - 5) : name;
		...
		void SelectedRemove(ListItem item)
		{
			...
			selectList.Remove(name.EndsWith(".meta") ? name.Substring(0, name.Length - 5) : name + ".meta");
