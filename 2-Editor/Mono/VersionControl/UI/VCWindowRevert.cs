	...
	internal class WindowRevert : EditorWindow
	{
		...
		void OnGUI ()
		{
			...
			if (assetList.Count > 0 && GUILayout.Button ("Revert"))
			{
				...
				{
					if ((a.path) == EditorApplication.currentScene)
					{
						bool continueRevert = EditorUtility.DisplayDialog ("Revert open scene?",
						                             "You are about to revert your currently open scene:\n\n" +
