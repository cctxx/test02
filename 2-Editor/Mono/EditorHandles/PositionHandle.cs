...
namespace UnityEditor
{
	public sealed partial class Handles
	{
		...
		static Vector3 DoPositionHandle_Internal(Vector3 position, Quaternion rotation)
		{
			...
			position = Slider(position, rotation * Vector3.right, size, ArrowCap, SnapSettings.move.x);

			color = isStatic? Color.Lerp(yAxisColor, staticColor, staticBlend) : yAxisColor;
			GUI.SetNextControlName("yAxis");
			position = Slider(position, rotation * Vector3.up, size, ArrowCap, SnapSettings.move.y);

			color = isStatic? Color.Lerp(zAxisColor, staticColor, staticBlend) : zAxisColor;
			GUI.SetNextControlName("zAxis");
			position = Slider(position, rotation * Vector3.forward, size, ArrowCap, SnapSettings.move.z);

			if (s_FreeMoveMode)
			{
				color = centerColor;
				GUI.SetNextControlName("FreeMoveAxis");
				position = FreeMoveHandle(position, rotation, size * .15f, SnapSettings.move, RectangleCap);
			}
			else
			{
		...
		static Vector3 DoPlanarHandle(PlaneHandle planeID, Vector3 position, Quaternion rotation, float handleSize)
		{
			...
			if (Camera.current.orthographic)
				cameraToTransformToolVector = handleTransform.inverse.MultiplyVector(SceneView.currentDrawingSceneView.cameraTargetRotation * -Vector3.forward).normalized;
			else
				cameraToTransformToolVector = handleTransform.inverse.MultiplyPoint (SceneView.currentDrawingSceneView.camera.transform.position).normalized;
