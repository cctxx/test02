...
namespace UnityEditor
{
	public sealed partial class Handles
	{
		internal static float DoRadiusHandle(Quaternion rotation, Vector3 position, float radius, bool handlesOnly)
		{
			...
			{
				planeNormal = position - Camera.current.transform.position; // vector from camera to center
			...
			for (int i = 0; i < 6; i++)
			{
				...
				// in order to not fuck up the id order when slider handles pop in and out.
				...
				if ((angle > 5 && angle < 175) || GUIUtility.hotControl == id)
				{
					...
					GUI.changed = false;
					pos = Slider1D.Do (id, pos, dirs[i], HandleUtility.GetHandleSize (pos) * 0.03f, DotCap, 0);
