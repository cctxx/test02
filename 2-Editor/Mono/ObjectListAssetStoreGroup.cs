...
internal partial class ObjectListArea
{
	...
	class AssetStoreGroup : Group
	{
		...
		AssetStorePreviewManager.CachedAssetStoreImage GetIconForAssetStoreAsset (AssetStoreAsset assetStoreResource)
		{
			...
			{
#if false
				bool onlyCached = (m_Owner.LastScrollTime + ObjectListArea.kDelayQueryAfterScroll) > EditorApplication.timeSinceStartup;
				if (onlyCached)	
					m_Owner.Repaint();
				AssetStorePreviewManager.CachedAssetStoreImage item = AssetStorePreviewManager.TextureFromUrl (assetStoreResource.staticPreviewURL, assetStoreResource.name, m_Owner.gridSize, s_Styles.resultsGridLabel, s_Styles.resultsGrid, onlyCached);
#endif
...
// 				else if (item.lastUsed < 0 && !onlyCached) // not waiting for server but simply no image present for this url
// 				{
// 					m_Content.image = InternalEditorUtility.GetIconForFile (assetStoreResource.name);
// 					DrawObjectIcon (position, m_Content.image);
// 				}
		...
		{
			// Assert.IsTrue(assetStoreResource != null);
