...
internal class ParticleSystemCurveEditor
{
	...
	internal class Styles
	{
		...
		public GUIContent optimizeCurveText = new GUIContent("", "Click to optimize curve. Optimized curves are defined by having at most 3 keys, with a key at both ends");
	...
	public void Init()
	{
		...
		m_CurveEditorSettings.wrapColor = new Color(0, 0, 0, 0); // Drop any wrap line rendering
		...
		hTS.color = new Color(0.0f, 0.0f, 0.0f, 0.2f);
		hTS.distLabel = 30;
		hTS.stubs = false;
		...
		vTS.color = new Color(0.0f, 0.0f, 0.0f, 0.2f);
		vTS.distLabel = 20;
		vTS.stubs = false;
	...
	public void AddCurveDataIfNeeded (string curveName, CurveData curveData)
	{
		...
		Vector3 c = InspectorState.GetVector3(curveName, Vector3.zero);
	...
	private bool Remove (int index)
	{
		if (index >= 0 && index < m_AddedCurves.Count)
		{
			...
			InspectorState.EraseVector3 (curveName);
			...
			for (int j = 0; j < m_AddedCurves.Count; ++j)
			{
				...
				if (cd.m_MaxId == topMostCurveID || cd.m_MinId == topMostCurveID)
				{
					if (doubleCurve.signedRange == cd.m_SignedRange)
					{
						if (cd.m_MaxId > 0)
						{
							SetCurve(m_CurveEditor.GetCurveFromID(cd.m_MaxId), doubleCurve.maxCurve);
						}
		
						if (cd.m_MinId > 0)
						{
							SetCurve(m_CurveEditor.GetCurveFromID(cd.m_MinId), doubleCurve.minCurve);
						}
					}
					else
					{
	...
	void DoOptimizeCurveButton (Rect rect)	
	{
		...
		if (selection.Count > 0)
		{
			...
			{
				CurveWrapper cw = selection[j].curveWrapper;
				numValidPolynomialCurve += AnimationUtility.IsValidPolynomialCurve(cw.curve) ? 1 : 0;
			}
			...
			if (selection.Count != numValidPolynomialCurve)
			{
				if (GUI.Button(buttonRect, s_Styles.optimizeCurveText, s_Styles.plus))
				{
					...
					{
						CurveWrapper cw = selection[j].curveWrapper;
						if (!AnimationUtility.IsValidPolynomialCurve(cw.curve))
						{
		...
		{ 
			...
			if (m_CurveEditor.GetTopMostCurveID(out topMostCurveID))
			{
				CurveWrapper cw = m_CurveEditor.getCurveWrapperById (topMostCurveID);
				if (!AnimationUtility.IsValidPolynomialCurve(cw.curve))
				{
					if (GUI.Button(buttonRect, s_Styles.optimizeCurveText, s_Styles.plus))
					{
										...
										CurveWrapper.GetAxisScalarsCallback getAxisScalarsCallback, CurveWrapper.SetAxisScalarsCallback setAxisScalarsCallback)
	{
		...
		cw.renderer.SetCustomRange(0.0f, rangeH);
