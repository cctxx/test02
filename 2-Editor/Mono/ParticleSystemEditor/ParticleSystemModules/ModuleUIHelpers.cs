...
partial class ModuleUI
{
	...
	static public float k_CompactFixedModuleWidth = 195f;
	...
	protected static Rect GetControlRect (int height)
	{
		...
		return GUILayoutUtility.GetRect (0, height, style);
	}
	...
	{
		float orgFloatValue = floatProp.floatValue;
		float floatValue = FloatDraggable (rect, orgFloatValue, remap, dragWidth, formatString);
		if (floatValue != orgFloatValue)
	...
	{
		bool orgValue = boolProp.boolValue;
		bool newValue = EditorGUI.Toggle(rect, orgValue, ParticleSystemStyles.Get().toggle);
	...
	public static void GUIToogleWithIntField(GUIContent guiContent, SerializedProperty boolProp, SerializedProperty intProp, bool invertToggle)
	{
		...
		{
			float dragWidth = 25f;
			Rect intDragRect = new Rect(rect.x + EditorGUIUtility.labelWidth + k_toggleWidth, rect.y, rect.width - k_toggleWidth, rect.height);
			intProp.intValue = IntDraggable(intDragRect, null, intProp.intValue, dragWidth);
		}
	}
	...
	public static void GUIObject (GUIContent label, SerializedProperty objectProp)
	{
		...
		EditorGUI.ObjectField (rect, objectProp, GUIContent.none, ParticleSystemStyles.Get().objectField); 
	}
		...
		foreach (Object obj in references)
		{
			if (obj != null)
			{
				...
				if (gameObj != null)
				{
					...
					if (s)
					{
						//if (ParticleEffectUtils.ValidateSubEmitter (s, m_ParticleSystemUI) == s)
						return s;
					}
				}
			}
	...
	public int GUIListOfFloatObjectToggleFields (GUIContent label, SerializedProperty[] objectProps, EditorGUI.ObjectFieldValidator validator, GUIContent buttonTooltip, bool allowCreation)
	{
		...
		Rect rect = GUILayoutUtility.GetRect (0, (kSingleLineHeight + 2) * numObjects);  // the +1 is that label is on it own line
		...
		for (int i=0; i<numObjects; ++i)
		{
			...
			int id = EditorGUIUtility.GetControlID(1235498, EditorGUIUtility.native, r2);
	...
	{
		Rect rect = GUILayoutUtility.GetRect (0, kSingleLineHeight);
		return IntDraggable (rect, label, intValue, EditorGUIUtility.labelWidth);
	}
	...
	public static int GUIInt (GUIContent guiContent, int intValue)
	{
		Rect rect = GUILayoutUtility.GetRect (0, kSingleLineHeight);
		PrefixLabel(rect, guiContent);
		int value = IntDraggable(rect, null, intValue, EditorGUIUtility.labelWidth);
		return value;
	}
	...
	public static void GUIMinMaxRange (GUIContent label, SerializedProperty vec2Prop)
	{
		...
		range.x = FloatDraggable(rect, range.x, 1f, kDragSpace, "g7");
		range.x = Mathf.Clamp(range.x, 0f, range.y - 0.01f);

		rect.x += floatFieldWidth + kDragSpace;
		range.y = FloatDraggable(rect, range.y, 1f, kDragSpace, "g7");
	...
	{
		floatProp.floatValue = EditorGUILayout.Slider (name, floatProp.floatValue * remap, a, b, GUILayout.MinWidth(300)) / remap;
	}
	public static void GUIMinMaxSlider (GUIContent label, SerializedProperty vec2Prop, float a, float b)
	{
		Rect rect = GetControlRect (kSingleLineHeight * 2); 
		Rect r = rect;
	...
	{
		NUnit.Framework.Assert.That (options.Length == 2);
		
		Rect rect = GetControlRect (kSingleLineHeight);
		rect = PrefixLabel(rect, label);
		
		int value = boolProp.boolValue ? 1 : 0;
		int newValue = EditorGUI.Popup(rect, value, options, ParticleSystemStyles.Get().popup);
		if (newValue != value)
		{
	...
	{
		int id = EditorGUIUtility.GetControlID(1321321231, EditorGUIUtility.native, position);
	...
	public void GUIMinMaxGradient(GUIContent label, SerializedMinMaxGradient minMaxGradient)
	{
		...
		bool useRandomness = state >= MinMaxGradientState.k_RandomBetweenTwoColors;

		Rect rect = GUILayoutUtility.GetRect (0, useRandomness ? 2 * kSingleLineHeight : kSingleLineHeight);
		Rect popupRect = GetPopupRect (rect);
		rect = SubtractPopupWidth (rect);
		...
		switch (state)
		{
			case MinMaxGradientState.k_Color:
				GUIColor (gradientRect, minMaxGradient.m_MaxColor);
			break;
			
			case MinMaxGradientState.k_RandomBetweenTwoColors:
				GUIColor (gradientRect, minMaxGradient.m_MaxColor);
				gradientRect.y += gradientRect.height;
				GUIColor (gradientRect, minMaxGradient.m_MinColor);
			break;
	...
	public void GUIMinMaxColor (GUIContent label, SerializedMinMaxColor minMaxColor)
	{
		...
		float colorAreaWidth = rect.width - k_minMaxToggleWidth - 5; 
		if (!minMaxColor.minMax.boolValue)
		{
			Rect colorRect = new Rect (rect.x, rect.y, colorAreaWidth, rect.height);
			GUIColor (colorRect, minMaxColor.maxColor);
		}
		else
		{
			Rect colorRect = new Rect(rect.x, rect.y, colorAreaWidth*0.5f-2, rect.height);
			GUIColor (colorRect, minMaxColor.minColor);
			colorRect.x += colorRect.width + 4;
			GUIColor (colorRect, minMaxColor.maxColor);
		}
		Rect minMaxButtonRect = new Rect(rect.xMax - k_minMaxToggleWidth, rect.y, k_minMaxToggleWidth, kSingleLineHeight);
		GUIMMColorPopUp (minMaxButtonRect, minMaxColor.minMax);
	}
	...
	public void GUITripleMinMaxCurve(GUIContent label, GUIContent x, SerializedMinMaxCurve xCurve, GUIContent y, SerializedMinMaxCurve yCurve, GUIContent z, SerializedMinMaxCurve zCurve, SerializedProperty randomizePerFrame)
	{
		...
		if (state == MinMaxCurveState.k_TwoScalars)
			numLines++;
		Rect rect = GetControlRect (kSingleLineHeight * numLines);
		Rect popupRect = GetPopupRect (rect);
		rect = SubtractPopupWidth (rect);
		Rect r = rect;
	...
	public static void GUIMMCurveStateList(Rect rect, SerializedMinMaxCurve[] minMaxCurves)
	{
		if (EditorGUI.ButtonMouseDown(rect, GUIContent.none, FocusType.Passive, ParticleSystemStyles.Get().minMaxCurveStateDropDown))
		{
			...
			{
				if(allowState[i])
					menu.AddItem(texts[i], (minMaxCurves[0].state == states[i]), SelectMinMaxCurveStateCallback, new CurveStateCallbackData (states[i], minMaxCurves));
			}
			menu.DropDown(rect);
			Event.current.Use();
		}
	...
	public static void GUIMMGradientPopUp (Rect rect, SerializedMinMaxGradient gradientProp)
	{
		if (EditorGUI.ButtonMouseDown(rect, GUIContent.none, FocusType.Passive, ParticleSystemStyles.Get().minMaxCurveStateDropDown))
		{
			...
			{
				if(allowState[i])
					menu.AddItem(texts[i], (gradientProp.state == states[i]), SelectMinMaxGradientStateCallback, new GradientCallbackData(states[i], gradientProp));
			}
			menu.ShowAsContext();
			Event.current.Use();
		}
