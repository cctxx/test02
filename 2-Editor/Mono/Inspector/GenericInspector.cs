...
internal class GenericInspector : Editor
{
	...
	internal override bool GetOptimizedGUIBlock (bool isDirty, bool isVisible, out OptimizedGUIBlock block, out float height)
	{
		...
		{
			if (AudioUtil.HaveAudioCallback(target as MonoBehaviour) && AudioUtil.GetCustomFilterChannelCount(target as MonoBehaviour) > 0)
			{
				return false;			
			}
		}
	...
	{
		if (target.GetType () != typeof(GUISkin) && target.GetType ().IsSubclassOf(typeof(ScriptableObject)))
		{
	...
	public bool MissingMonoBehaviourGUI ()
	{
		...
		{
			GUIContent c = EditorGUIUtility.TextContent ("GenericInspector.ScriptIsInvalid");			
