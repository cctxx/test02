...
internal class ScriptExecutionOrderInspector : Editor
{
	...
	*/
	...
	{
		public SortMonoScriptNameOrder ()
		{}
	...
	public class Styles
	{
		public GUIContent helpText = EditorGUIUtility.TextContent ("ScriptExecutionOrderInspector.HelpText");
		...
		public GUIContent defaultTimeContent = EditorGUIUtility.TextContent ("ScriptExecutionOrderInspector.DefaultTime");
	...
	private void Apply ()
	{
		...
		if (!IsOpenForEdit() && Provider.enabled) {
			var task = Provider.Checkout (changedScripts.ToArray (), CheckoutMode.Both);
	...
	private void ShowScriptPopup (Rect r)
	{
		...
		{
			names[c] = m_DefaultTimeScripts[c].name;
			enabled[c] = true;
		}
	...
	public override void OnInspectorGUI ()
	{
		...
		EditorGUILayout.BeginVertical (EditorStyles.inspectorFullWidthMargins);
		{
			...
			Rect listRect = EditorGUILayout.BeginVertical ();
			{
				...
				MonoScript dropped = EditorGUI.DoDropField (listRect, dropFieldId, typeof (MonoScript), null, false, m_Styles.dropField) as MonoScript;
				if (dropped)
				...
				GUILayout.BeginHorizontal (m_Styles.toolbar);
				{
					...
					if (EditorGUI.ButtonMouseDown (r2, content, FocusType.Native, m_Styles.toolbarDropDown))
						ShowScriptPopup (r2);
				} GUILayout.EndHorizontal ();
	...
	public void DrawElement (Rect r, object obj, bool dragging)
	{
		...
		GUI.Label (GetButtonLabelRect (r), script.name);

		int oldNr = GetExecutionOrder (script);
		string intStr = EditorGUI.DelayedTextField (GetFieldRect (r), oldNr.ToString (), "0123456789-", EditorStyles.textField);
		int newNr = oldNr;
