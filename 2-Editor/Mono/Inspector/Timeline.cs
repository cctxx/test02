...
namespace UnityEditor
{
	internal class Timeline
	{
		...
		private enum DragStates { None, LeftSelection, RightSelection, FullSelection, Destination, Source, Playhead };
		...
		private Vector3[] GetPivotVectors(PivotSample[] samples, float motionStart, float motionStop, Rect rect, bool loop)
		{
			if (samples.Length == 0 || (motionStop - motionStart) < 0.33f) // not pivots for super small anims
				return new Vector3[0];
			float width = motionStop - motionStart;
			...
			for (int i = 0; i < samples.Length; i++)
			{
				...
				point.y = rect.height / 16 + s.m_Weight * 12 * rect.height / 16;
			...
			if (loop && vectorList[vectorList.Count - 1].x <= rect.width)
			{
				...
				loopVectors.Add(vectorList[vectorList.Count - 1]);
		...
		private void DoPivotCurves ()
		{
			...
			if (m_SrcPivotVectors == null)
				m_SrcPivotVectors = GetPivotVectors (m_SrcPivotList.ToArray(), srcStart, srcStop, rect, srcLoop);
			if (m_DstPivotVectors == null)
				m_DstPivotVectors = GetPivotVectors (m_DstPivotList.ToArray(), dstStart, dstStop, rect, dstLoop);
		...
		{
            // disabling constraints, it breaks the transition setup workflow

            /* 
			Rect r = new Rect(0, 0, m_Rect.width, 150);
			// the right start time should not be greater than the left stop time	
			if (m_TimeArea.TimeToPixel (DstStartTime, r) + m_DstDragOffset > 
				m_TimeArea.TimeToPixel (SrcStopTime, r))
			{
				float limit = m_TimeArea.TimeToPixel(SrcStopTime, r) - m_TimeArea.TimeToPixel (DstStartTime, r);
				m_DstDragOffset = Mathf.Min (limit, m_DstDragOffset);
			}
			
			// the right stop time should not be less than the left start time
			if (m_TimeArea.TimeToPixel (DstStopTime, r) + m_DstDragOffset < 
				m_TimeArea.TimeToPixel (SrcStartTime, r))
			{
				
				float limit = m_TimeArea.TimeToPixel(SrcStartTime, r) - m_TimeArea.TimeToPixel (DstStopTime, r);
				m_DstDragOffset = Mathf.Max (limit, m_DstDragOffset);
			}
			
			
			if (m_DragState == DragStates.LeftSelection || m_DragState == DragStates.FullSelection)
			{
				// the start shouldn't be able to be dragged past the beginning of the second clip
				float limit =  m_TimeArea.TimeToPixel (DstStartTime, r) - m_TimeArea.TimeToPixel(TransitionStartTime, r);
				if (m_LeftThumbOffset < limit)
				{
					m_LeftThumbOffset = limit;
					if (m_DragState == DragStates.FullSelection)
						m_RightThumbOffset = limit;
				}
				
				// the start shouldn't be able to be dragged past the end of either clip
				limit = m_TimeArea.TimeToPixel (SrcStopTime, r) - m_TimeArea.TimeToPixel (TransitionStartTime, r);
				limit = Mathf.Min (limit, m_TimeArea.TimeToPixel (DstStopTime, r) - m_TimeArea.TimeToPixel (TransitionStartTime, r));
				if (m_LeftThumbOffset > limit)
				{
					m_LeftThumbOffset = limit;
					if (m_DragState == DragStates.FullSelection)
				...
				// the left transition point shouldn't be able to be dragged past the right one
				if (m_DragState == DragStates.LeftSelection)
				{
					limit =  m_TimeArea.TimeToPixel (TransitionStopTime, r) - m_TimeArea.TimeToPixel(TransitionStartTime, r);
					if (m_LeftThumbOffset > limit)
						m_LeftThumbOffset = limit;
			...
			{
				// the right transition point shouldn't be able to be dragged past the left one
				float limit = m_TimeArea.TimeToPixel(TransitionStartTime, r) - m_TimeArea.TimeToPixel (TransitionStopTime, r);
				m_RightThumbOffset = Mathf.Max (limit, m_RightThumbOffset);
				
				// the right transition can't be dragged past the width of the reference clip (!?!?!?)
			...
			{
				// the destination clip should be able to be dragged past the transition start
				float limit =  m_TimeArea.TimeToPixel(TransitionStartTime, r) - m_TimeArea.TimeToPixel (DstStartTime, r);
				m_DstDragOffset = Mathf.Min (limit, m_DstDragOffset);
				
				// the end if teh destination clip shouldn't be able to be less than the transition start
			 	limit = m_TimeArea.TimeToPixel (TransitionStartTime, r) - m_TimeArea.TimeToPixel (DstStopTime, r);
				m_DstDragOffset = Mathf.Max (limit, m_DstDragOffset);
		...
		public bool DoTimeline(Rect timeRect)
		{
			...
			if (timeAreaStart != StartTime)
			{
				StartTime = timeAreaStart;
				GUI.changed = true;
			}	
			if (timeAreaStop != StopTime)
			{
			...
			Time = (Time < StartTime)? StartTime : Time;
			Time = (Time > StopTime)? StartTime : Time;
			...
			GUI.BeginGroup (timeRect);
				...
				float srcStart = m_TimeArea.TimeToPixel (SrcStartTime, timeRect);
				float srcStop = m_TimeArea.TimeToPixel (SrcStopTime, timeRect);
				float dstStart = m_TimeArea.TimeToPixel (DstStartTime, timeRect) + m_DstDragOffset;
				float dstStop = m_TimeArea.TimeToPixel (DstStopTime, timeRect) + m_DstDragOffset;
				float transStart = m_TimeArea.TimeToPixel (TransitionStartTime, timeRect) + m_LeftThumbOffset;
				float transStop = m_TimeArea.TimeToPixel (TransitionStopTime, timeRect) + m_RightThumbOffset;
				float playPoint = m_TimeArea.TimeToPixel (Time, timeRect);
				...
				Rect transRect = new Rect(transStart, 18, transStop - transStart, timeRect.height - 18);
				...
				if (evt.type == EventType.MouseDrag)
				{
					if (EditorGUIUtility.hotControl == id)
					{
						...
						case DragStates.Playhead:
							Time = m_TimeArea.PixelToTime (playPoint + evt.delta.x, timeRect);
							break;
				...
				if (evt.type == EventType.MouseUp)
				{
					if (EditorGUIUtility.hotControl == id)
					{
						...
						SrcStartTime = m_TimeArea.PixelToTime(srcStart, timeRect);
						SrcStopTime = m_TimeArea.PixelToTime(srcStop, timeRect);
						DstStartTime = m_TimeArea.PixelToTime(dstStart, timeRect);
						DstStopTime = m_TimeArea.PixelToTime(dstStop, timeRect); 
						TransitionStartTime = m_TimeArea.PixelToTime(transStart, timeRect);
						TransitionStopTime = m_TimeArea.PixelToTime(transStop, timeRect);
						GUI.changed = true;
						m_DragState = DragStates.None;
				...
				Handles.DrawLine (new Vector3(playPoint, 19, 0), new Vector3(playPoint, timeRect.height, 0));
