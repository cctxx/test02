...
internal class AddComponentWindow : EditorWindow
{
	...
	class ComponentElement : Element
	{
		...
		public ComponentElement (int level, string name, string menuPath, string commandString)
		{
			...
			if (commandString.StartsWith("SCRIPT"))
			{
				...
				string path = AssetDatabase.GetAssetPath (scriptID);
	...
	class NewScriptElement : GroupElement
	{
		...
		public void Create ()
		{
			...
			foreach (GameObject go in gameObjects)
			{
				...
				InternalEditorUtility.AddScriptComponentUnchecked (go, script);
			}
		...
		{ get {
			...
			{
				case Language.JavaScript:
					return Path.Combine (basePath, "80-Javascript-NewBehaviourScript.js.txt");
				case Language.CSharp:
					return Path.Combine (basePath, "81-C# Script-NewBehaviourScript.cs.txt");
				case Language.Boo:
					return Path.Combine (basePath, "82-Boo Script-NewBehaviourScript.boo.txt");
				default:
					throw new ArgumentOutOfRangeException ();
			}
		} }
	...
	class Styles
	{
		...
		public Styles ()
		{
			...
			//componentButton.normal.textColor = header.normal.textColor;
			//componentButton.font = EditorStyles.label.font;
			componentButton.padding.left -= 15;
			componentButton.fixedHeight = 20;
			//componentButton.wordWrap = false;
	...
	private const int kHeaderHeight = 40;
	...
	void OnEnable ()
	{
		...
		s_Lang = (Language)EditorPrefs.GetInt (kLanguageEditorPrefName, 0);
	...
	private void CreateComponentTree ()
	{
		...
		for (int i=0; i<menus.Length; i++)
		{
			...
			while (path.Length-1 > stack.Count)
			{
				tree.Add (new GroupElement (stack.Count, path[stack.Count]));
				stack.Add (path[stack.Count]);
			}
			tree.Add (new ComponentElement (stack.Count, path[path.Length-1], str, commands[i]));
		}
	...
	internal void OnGUI ()
	{
		...
		EditorGUI.BeginDisabledGroup (activeParent is NewScriptElement);
		string newSearch = EditorGUI.SearchField (searchRect, m_Search);
		EditorGUI.EndDisabledGroup ();
		...
		// Animate
		if (m_Anim != m_AnimTarget && Event.current.type == EventType.Repaint)
		{
	...
	private void ListGUI (Element[] tree, GroupElement parent)
	{
		...
		for (int i=0; i<children.Count; i++)
		{
			...
			if (Event.current.type == EventType.Repaint)
			{
				GUIStyle labelStyle = (e is ComponentElement)? s_Styles.componentButton : s_Styles.groupButton;
	...
	/*private void DrawHelpArea (Rect rect)
	{
		Element e = activeElement;
		GUILayout.BeginArea (rect, s_Styles.previewBackground);
		
		if (e != null && e is ComponentElement)
		{
			GUILayout.Label (e.content, s_Styles.previewHeader);
			GUILayout.Label ("The "+e.name+" component is useful for many things.", s_Styles.previewText);
		}
