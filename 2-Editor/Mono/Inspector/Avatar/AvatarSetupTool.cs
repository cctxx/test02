...
namespace UnityEditor
{
	internal static class AvatarSetupTool
	{
		...
		internal class BoneWrapper
		{
			...
			public void BoneDotGUI(Rect rect, int boneIndex, bool doClickSelect, bool doDragDrop, SerializedObject serializedObject)
			{
				int id = GUIUtility.GetControlID (FocusType.Passive, rect);
				
				if (doClickSelect)
					HandleClickSelection (rect, boneIndex);
				
				if (doDragDrop)
					HandleDragDrop(rect, boneIndex, id, serializedObject);
			...
			private void HandleDragDrop(Rect dropRect, int boneIndex, int id, SerializedObject serializedObject)
			{
		...
		{ 
			 ...
			 (int)HumanBodyBones.Head,
			 (int)HumanBodyBones.Head,
			 (int)HumanBodyBones.LeftHand, (int)HumanBodyPart.LeftFingerStart + (int)HumanFingerBones.ThumbProximal, (int)HumanBodyPart.LeftFingerStart + (int)HumanFingerBones.ThumbIntermediate,
			 (int)HumanBodyBones.LeftHand, (int)HumanBodyPart.LeftFingerStart + (int)HumanFingerBones.IndexProximal, (int)HumanBodyPart.LeftFingerStart + (int)HumanFingerBones.IndexIntermediate,
			 (int)HumanBodyBones.LeftHand, (int)HumanBodyPart.LeftFingerStart + (int)HumanFingerBones.MiddleProximal, (int)HumanBodyPart.LeftFingerStart + (int)HumanFingerBones.MiddleIntermediate, 
			 (int)HumanBodyBones.LeftHand, (int)HumanBodyPart.LeftFingerStart + (int)HumanFingerBones.RingProximal, (int)HumanBodyPart.LeftFingerStart + (int)HumanFingerBones.RingIntermediate,
			 (int)HumanBodyBones.LeftHand, (int)HumanBodyPart.LeftFingerStart + (int)HumanFingerBones.LittleProximal, (int)HumanBodyPart.LeftFingerStart + (int)HumanFingerBones.LittleIntermediate,
			 (int)HumanBodyBones.RightHand, (int)HumanBodyPart.RightFingerStart + (int)HumanFingerBones.ThumbProximal, (int)HumanBodyPart.RightFingerStart + (int)HumanFingerBones.ThumbIntermediate,
			 (int)HumanBodyBones.RightHand, (int)HumanBodyPart.RightFingerStart + (int)HumanFingerBones.IndexProximal, (int)HumanBodyPart.RightFingerStart + (int)HumanFingerBones.IndexIntermediate,
			 (int)HumanBodyBones.RightHand, (int)HumanBodyPart.RightFingerStart + (int)HumanFingerBones.MiddleProximal, (int)HumanBodyPart.RightFingerStart + (int)HumanFingerBones.MiddleIntermediate, 
			 (int)HumanBodyBones.RightHand, (int)HumanBodyPart.RightFingerStart + (int)HumanFingerBones.RingProximal, (int)HumanBodyPart.RightFingerStart + (int)HumanFingerBones.RingIntermediate,
			 (int)HumanBodyBones.RightHand, (int)HumanBodyPart.RightFingerStart + (int)HumanFingerBones.LittleProximal, (int)HumanBodyPart.RightFingerStart + (int)HumanFingerBones.LittleIntermediate,
		};
		...
		private static BonePoseData[] sBonePoses = new BonePoseData[]
		{
			...
			new BonePoseData (Vector3.up, true, 30, new int[] {(int)HumanBodyBones.Chest, (int)HumanBodyBones.Neck, (int)HumanBodyBones.Head}), // Spine,
			new BonePoseData (Vector3.up, true, 30, new int[] {(int)HumanBodyBones.Neck, (int)HumanBodyBones.Head}), // Chest,
			...
			new BonePoseData (-Vector3.right, false, 10, Vector3.forward, new int[] {(int)HumanBodyPart.LeftFingerStart + (int)HumanFingerBones.MiddleProximal}), // LeftHand,
			new BonePoseData ( Vector3.right, false, 10, Vector3.forward, new int[] {(int)HumanBodyPart.RightFingerStart + (int)HumanFingerBones.MiddleProximal}), // RightHand,
			null, // LeftToes,
			null, // RightToes,
			null, // LeftEye,
			null, // RightEye,
		...
		public static void AutoSetup (GameObject modelPrefab, GameObject modelInstance, SerializedObject modelImporterSerializedObject)
		{
			...
			Dictionary<int, Transform> mapping = AvatarAutoMapper.MapBones (modelInstance.transform, modelBones);
			SimpleProfiler.End ();
		...
		public static SerializedProperty FindSkeletonBone (SerializedProperty skeletonBoneArray, Transform t, bool createMissing, bool isRoot)
		{
			...
			if (createMissing)
			{
				...
				if (bone != null)
				{
					...
					bone.FindPropertyRelative (sTransformModified).boolValue = true;
					return bone;
				}
			}
			return null;
		...
		{
			CopyPose (go.transform, source.transform);
		}
		...
		{
			// Create only missing bone if the avatar has not been created yet
			// otherwise the importer should create the skeleton bone
			// @TODO@MECANIM: Find a way to avoid creating missing bones if the user didn't make any changes.
			//        Currently the editor will always claim that changes were made even if the user did nothing.
			...
			skeletonBone.transformModified = 1;

			ArrayUtility.Add(ref skeletonBones, skeletonBone);

			/*
			SimpleProfiler.Begin ("FindSkeletonBone");
			SerializedProperty bone = FindSkeletonBone (serializedObject, transform, true, isRoot);
			SimpleProfiler.End ();
			if (bone != null)
			{
				SimpleProfiler.Begin ("FindPropertyRelative");
				...
				SerializedProperty modifiedP = bone.FindPropertyRelative (sTransformModified);
				SimpleProfiler.End ();
				
				SimpleProfiler.Begin ("CompareTransformToDescription");
				if (positionP != null && positionP.vector3Value != transform.localPosition)
				{
					positionP.vector3Value = transform.localPosition;
					modifiedP.boolValue = true;
				}
				
				if (rotationP != null && rotationP.quaternionValue != transform.localRotation)
				{
					rotationP.quaternionValue = transform.localRotation;
					modifiedP.boolValue = true;
				}
				
				if (scaleP != null && scaleP.vector3Value != transform.localScale)
				{
					scaleP.vector3Value = transform.localScale;
					modifiedP.boolValue = true;
				}
				SimpleProfiler.End ();
			}
		...
		public static float GetBoneAlignmentError (BoneWrapper[] bones, Quaternion avatarOrientation, int boneIndex)
		{
			...
			if (pose.planeNormal != Vector3.zero)
				dir = dir - Vector3.Project (dir, space * pose.planeNormal);
		...
		private static Quaternion GetRotationSpace (BoneWrapper[] bones, Quaternion avatarOrientation, int boneIndex)
		{
			...
			{
				int parentIndex = sHumanParent[boneIndex];
