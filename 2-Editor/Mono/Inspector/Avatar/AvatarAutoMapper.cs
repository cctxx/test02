...
namespace UnityEditor
{
	internal class AvatarAutoMapper
	{
		...
		private class BoneMatch : IComparable<BoneMatch>
		{
			...
			public BoneMatch humanBoneParent { get {
				BoneMatch p = parent;
		...
		private static BoneMappingItem[] s_MappingDataBody = new BoneMappingItem[]
		{
			...
			new BoneMappingItem ((int)HumanBodyBones.Chest,			(int)HumanBodyBones.RightShoulder,	1, 3, 0.0f, Vector3.right, Side.Right, true, false, kShoulderKeywords),
			...
			new BoneMappingItem ((int)HumanBodyBones.Chest,			(int)HumanBodyBones.Neck,			1, 3, 1.8f, Vector3.up, Side.None, true, false, kNeckKeywords),
		...
		private static BoneMappingItem[] s_MappingDataHand = new BoneMappingItem[]
		{
			new BoneMappingItem (-2,										-1,											1, 2, 0.0f, Side.None),
			
			new BoneMappingItem (-1,										(int)HumanFingerBones.ThumbProximal,		1, 3, 0.0f, new Vector3 (2,0, 1), Side.None, kThumbKeywords),
			new BoneMappingItem (-1,										(int)HumanFingerBones.IndexProximal,		1, 3, 0.0f, new Vector3 (4,0, 1), Side.None, kIndexFingerKeywords),
			new BoneMappingItem (-1,										(int)HumanFingerBones.MiddleProximal,		1, 3, 0.0f, new Vector3 (4,0, 0), Side.None, kMiddleFingerKeywords),
			new BoneMappingItem (-1,										(int)HumanFingerBones.RingProximal,			1, 3, 0.0f, new Vector3 (4,0,-1), Side.None, kRingFingerKeywords),
			new BoneMappingItem (-1,										(int)HumanFingerBones.LittleProximal,		1, 3, 0.0f, new Vector3 (4,0,-2), Side.None, kLittleFingerKeywords),
			
			new BoneMappingItem ((int)HumanFingerBones.ThumbProximal,		(int)HumanFingerBones.ThumbIntermediate,	1, 1, 0.0f, Side.None, false, true),
			new BoneMappingItem ((int)HumanFingerBones.IndexProximal,		(int)HumanFingerBones.IndexIntermediate,	1, 1, 0.0f, Side.None, false, true),
			new BoneMappingItem ((int)HumanFingerBones.MiddleProximal,		(int)HumanFingerBones.MiddleIntermediate,	1, 1, 0.0f, Side.None, false, true),
			new BoneMappingItem ((int)HumanFingerBones.RingProximal,		(int)HumanFingerBones.RingIntermediate,		1, 1, 0.0f, Side.None, false, true),
			new BoneMappingItem ((int)HumanFingerBones.LittleProximal,		(int)HumanFingerBones.LittleIntermediate,	1, 1, 0.0f, Side.None, false, true),
			
			new BoneMappingItem ((int)HumanFingerBones.ThumbIntermediate,	(int)HumanFingerBones.ThumbDistal,			1, 1, 0.0f, Side.None, false, true),
			new BoneMappingItem ((int)HumanFingerBones.IndexIntermediate,	(int)HumanFingerBones.IndexDistal,			1, 1, 0.0f, Side.None, false, true),
			new BoneMappingItem ((int)HumanFingerBones.MiddleIntermediate,	(int)HumanFingerBones.MiddleDistal,			1, 1, 0.0f, Side.None, false, true),
			new BoneMappingItem ((int)HumanFingerBones.RingIntermediate,	(int)HumanFingerBones.RingDistal,			1, 1, 0.0f, Side.None, false, true),
			new BoneMappingItem ((int)HumanFingerBones.LittleIntermediate,	(int)HumanFingerBones.LittleDistal,			1, 1, 0.0f, Side.None, false, true),
		};
		...
		private static int GetLeftBoneIndexFromRight (int rightIndex)
		{
			...
			{
				return rightIndex + (int)HumanBodyPart.LeftFingerStart - (int)HumanBodyPart.RightFingerStart;
			}
		}
		...
		public static void InitGlobalMappingData ()
		{
			...
			for (int i=0; i<s_MappingDataHand.Length; i++)
				s_MappingDataHand[i].GetChildren (s_MappingDataHand);
		...
		public Dictionary<int, Transform> MapBones (Transform root)
		{
			...
			if (mapping.ContainsKey ((int)HumanBodyBones.LeftHand))
			{
				...
				m_MappingData = s_MappingDataHand;
				m_MappingIndexOffset = (int)HumanBodyPart.LeftFingerStart;
				m_BoneMatchDict.Clear ();
				...
				{
					for (int i=0; i<(int)HumanFingerBones.LastBone; i++)
						mapping.Remove ((int)HumanBodyPart.LeftFingerStart + i);
				}
			}
			...
			if (mapping.ContainsKey ((int)HumanBodyBones.RightHand))
			{
				...
				m_MappingData = s_MappingDataHand;
				m_MappingIndexOffset = (int)HumanBodyPart.RightFingerStart;
				m_BoneMatchDict.Clear ();
				BoneMatch rootMatch = new BoneMatch (null, lowerArm, m_MappingData[0]);
				m_TreatDummyBonesAsReal = true;
				...
				{
					for (int i=0; i<(int)HumanFingerBones.LastBone; i++)
						mapping.Remove ((int)HumanBodyPart.RightFingerStart + i);
				}
			}
		...
		{
			if (match.doMap)
				mapping[match.item.bone + m_MappingIndexOffset] = match.bone;
		...
		private int GetMatchKey (BoneMatch parentMatch, Transform t, BoneMappingItem goalItem)
		{
			...
			int key = goalItem.bone + m_MappingIndexOffset;
		...
		private string GetNameOfBone (int boneIndex)
		{
			if (boneIndex < 0)
				return ""+boneIndex;
			else if (m_MappingIndexOffset == 0)
				return ""+(HumanBodyBones)boneIndex;
			else
				return ""+(HumanFingerBones)boneIndex;
		}
		...
		{
			foreach (BoneMappingItem i in m_MappingData)
				if (i.bone == bone)
					return i;
		...
		private void EvaluateBoneMatch (BoneMatch match, bool confirmedChoice)
		{
			...
			if (match.item.dir != Vector3.zero)
			{
				...
				if (m_MappingIndexOffset >= (int)HumanBodyPart.LeftFingerStart && m_MappingIndexOffset < (int)HumanBodyPart.RightFingerStart)
					goalDir.x *= -1;
			...
			if (match.score > 0)
			{
				...
				{
					/*int validChildBones = 0;
					foreach (Transform child in match.bone)
						if (m_ValidBones.ContainsKey (child))
							validChildBones++;
					
					if (validChildBones == 0)*/
					{
