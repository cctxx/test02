...
internal partial class ObjectListArea
{
	...
	class LocalGroup : Group
	{
		...
		protected override void HandleUnusedDragEvents (float yOffset)
		{
			...
			//GUI.Label (localGroupRect, GUIContent.none, EditorStyles.helpBox);
		...
		{
			Assert.IsTrue((	filterItem != null && builtinResource == null) || 
			...
			Rect foldoutRect = new Rect (position.x + 2, position.y, 16,16); // ListMode foldout
			...
			float contentStartX = k_ListModeLeftPadding;
			if (ListMode)
			{
				itemRect.x = k_ListModeLeftPadding;
		...
		private static Rect ActualImageDrawPosition(Rect position, float imageWidth, float imageHeight)
		{
			if (imageWidth > position.width || imageHeight > position.height)
			{
				...
				//float imageAspect = (float) m_Content.image.width / (float) m_Content.image.height;
		...
		void RefreshBuiltinResourceList (SearchFilter searchFilter)
		{
			...
			{
				int classID = BaseObjectTools.StringToClassIDCaseInsensitive (className);
				if (classID >= 0)
					requiredClassIDs.Add (classID);
			}
			if (requiredClassIDs.Count > 0)
			{
				foreach (KeyValuePair<string, BuiltinResource[]> kvp in m_BuiltinResourceMap)
				{
					int classID = BaseObjectTools.StringToClassID (kvp.Key);
					foreach (int requiredClassID in requiredClassIDs)
					{
						if (BaseObjectTools.IsDerivedFromClassID (classID, requiredClassID))
		...
		private void InitBuiltinAssetType(System.Type type)
		{
			...
			int classID = BaseObjectTools.StringToClassID (typeName);
			if (classID < 0)
			{
			...
			BuiltinResource[] resourceList = EditorGUIUtility.GetBuiltinResourceList(classID);
		...
		public int IndexOfNewText (string newText, bool isCreatingNewFolder, bool foldersFirst)
		{
			...
			for (; idx < m_FilteredHierarchy.results.Length; ++idx)
			{
				...
				if (EditorUtility.SemiNumericCompare(System.IO.Path.GetFileNameWithoutExtension(propertyPath), newText) > 0)
				{
					return idx;
				}
			}
