	...
	internal class AnimationClipInfoProperties
	{
		...
		public void MaskFromClip(AvatarMask mask)
		{
			...
			if(bodyMask != null && bodyMask.isArray)
			{
				...
				{
					mask.SetHumanoidBodyPartActive(i, bodyMask.GetArrayElementAtIndex(i).intValue != 0);
				}
			}
		...
		public AnimationCurve GetCurve(int index)
		{
			...
			if (curves != null && curves.isArray)
			{
				ret = curves.GetArrayElementAtIndex(index).FindPropertyRelative("curve").animationCurveValue;
			}
		...
		public void SetCurve(int index, AnimationCurve curve)
		{
			...
			if (curves != null && curves.isArray)
			{
				curves.GetArrayElementAtIndex(index).FindPropertyRelative("curve").animationCurveValue = curve;
			}
		}
		...
		public void AddCurve()
		{
			...
			if (curves != null && curves.isArray)
			{
				...
				curves.GetArrayElementAtIndex(curves.arraySize-1).FindPropertyRelative("curve").animationCurveValue = new AnimationCurve(new Keyframe(0, 0), new Keyframe(1, 0));
			}
		}
