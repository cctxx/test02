...
internal class PresetLibraryEditorState
{
	...
	public string m_CurrrentLibrary = PresetLibraryLocations.defaultPresetLibrary;
...
internal partial class PresetLibraryEditor<T> where T : PresetLibrary
{
	class Styles
	{
		...
		public GUIContent plusButtonTextNotCheckedOut = new GUIContent("", "To add presets you need to press the 'Checkout' button below");
	...
	public System.Action<PresetLibrary> defaultLibraryWasCreated;
	...
	void VersionControlArea(Rect rect)
	{
		...
		{
			Provider.Checkout(new[] { pathWithExtension }, CheckoutMode.Both).SetCompletionAction(CompletionAction.UpdatePendingWindow);
		}
	}
	...
	public T GetCurrentLib ()
	{
		...
		if (lib == null)
		{
			...
			lib = PresetLibraryManager.instance.GetLibrary<T>(m_SaveLoadHelper, PresetLibraryLocations.defaultPresetLibrary);
			if (lib == null)
			{
				lib = CreateNewLibrary(PresetLibraryLocations.defaultPresetLibrary);
				if (lib != null)
				{
					// Add default set of presets
					if (defaultLibraryWasCreated != null)
					{
						defaultLibraryWasCreated(lib);
						PresetLibraryManager.instance.SaveLibrary(m_SaveLoadHelper, lib, PresetLibraryLocations.defaultPresetLibrary);
					}
				}
				else
				{
			...
			currentLibraryWithoutExtension = PresetLibraryLocations.defaultPresetLibrary;
		}
		return lib;
	}
