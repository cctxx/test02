...
internal class AnnotationWindow : EditorWindow
{
	...
	GUIContent icon3dGizmoContent = new GUIContent("3D Gizmos");
	...
	private void SyncToState()
	{
		...
		for (int i = 0; i < a.Length; ++i)
		{
			...
			if (anno.m_ScriptClass == "")
			{
				...
				if (s_Debug)
					debuginfo += "   " + BaseObjectTools.ClassIDToString(anno.m_ClassID) + ": icon " + anno.m_IconEnabled + " gizmo " + anno.m_GizmoEnabled + "\n";
			}
			else
			{
	...
	internal void OnGUI ()
	{
		...
		GUI.Label(new Rect(0, 0, position.width, position.height), GUIContent.none, m_Styles.background);
	...
	void DrawTopSection (float topSectionHeight)
	{
		...
		// Toggle 3D gizmos
		Rect toggleRect = new Rect (margin-2, curY, 80, 20);
		...
		if (s_Debug)
		{
			Rect texelSizeRect = new Rect(0, curY + 10, position.width - margin, 20);
		...
		{
			// Slider
			float sliderWidth = 160;
			float iconSize01 = ConvertTexelWorldSizeTo01(texelWorldSize);
			Rect sliderRect = new Rect (position.width-margin-sliderWidth , curY, sliderWidth, 20);
		...
		EditorGUI.BeginDisabledGroup(m_IsGameView);
		// Show Grid
		toggleRect = new Rect(margin - 2, curY, 80, 20);
...
internal class AInfo : System.IComparable, System.IEquatable<AInfo>
{
	...
	public AInfo (bool gizmoEnabled, bool iconEnabled, int flags, int classID, string scriptClass)
	{
		...
		m_Flags = flags;
		if (m_ScriptClass == "")
			m_DisplayText = BaseObjectTools.ClassIDToString(m_ClassID);
		else
