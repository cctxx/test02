...
namespace UnityEditorInternal
{
	internal class Chart
	{
		...
		public int DoGUI (ChartType type, int selectedFrame, ChartData cdata, bool active, GUIContent icon, out ChartAction action)
		{
			...
			int chartControlID = GUIUtility.GetControlID(s_ChartHash, FocusType.Keyboard);
			...
			Event evt = Event.current;
			EventType evtType = evt.GetTypeForControl(chartControlID);
			...
			if (m_DragItemIndex == -1)
				selectedFrame = HandleFrameSelectionEvents(selectedFrame, chartControlID, r, cdata, len);
			...
			{
				sideRect.y += 10;
				LabelDraggerDrag(chartControlID, type, cdata, sideRect, active);
			}
		...
		private void DrawSelectedFrame (int selectedFrame, ChartData cdata, Rect r)
		{
			if (selectedFrame - cdata.firstSelectableFrame >= 0)
			{
				...
				GL.Vertex3 (r.x + r.width / len * selectedFrame, r.y+1, 0);
				GL.Vertex3 (r.x + r.width / len * selectedFrame + r.width / len, r.y+1, 0);

				GL.Color (new Color (1, 1, 1, 0.7f));
				GL.Vertex3 (r.x + r.width / len * selectedFrame + r.width / len, r.yMax, 0);
				GL.Vertex3 (r.x + r.width / len * selectedFrame, r.yMax, 0);
				GL.End ();
			}
		}
		...
		private void DrawLabelsStacked(int selectedFrame, ChartData cdata, Rect r)
		{
			...
			float xpos = r.x + r.width / length * selectedFrame;
			...
			for (int s = 0; s < cdata.charts.Length; ++s)
			{
				if (heights[s] > 0)
				{
					...
					float alignment = (index & 1) == 0 ? -1.05f : 0.05f;
					DoLabel(xpos, r.y + r.height - ypositions[s] - 8, cdata.selectedLabels[index], alignment);
				}
			}
		...
		private void DrawLabelsLine (int selectedFrame, ChartData cdata, Rect r)
		{
			...
			for (int s = 0; s < cdata.charts.Length; ++s)
			{
				if (heights[s] > 0)
				{
					...
					// place odd labels on one side, even labels on another side
					float alignment = (s & 1) == 0 ? -1.05f : 0.05f;
					DoLabel(xpos, r.y + r.height - ypositions[s] - 8, cdata.selectedLabels[s], alignment);
				}
			}
			GUI.contentColor = Color.white;
		}
		...
		void DrawLabelDragger (ChartType type, Rect r, ChartData cdata)
		{
			...
			else
			{
				for (int i = 0; i < cdata.charts.Length; ++i)
				{
					Rect pos = new Rect (r.x, r.y + kDistFromTopToFirstLabel + i * kLabelHeight, kSideWidth, kLabelHeight);
					GUI.backgroundColor = cdata.charts[i].color;
					GUI.Label (pos, cdata.charts[i].name, ms_Styles.paneSubLabel);
				}
			}
		...
		private void LabelDraggerDrag (int chartControlID, ChartType chartType, ChartData cdata, Rect r, bool active)
		{
			...
			for (int i = cdata.charts.Length - 1; i >= 0; --i, ++idx)
			{
				...
				{	
					var togglePos = new Rect(r.x + 10 + kLabelXOffset, r.y + kDistFromTopToFirstLabel + idx * kLabelHeight, 9, 9);
