...
namespace TreeEditor
{
	public class TreeEditorHelper
	{
		...
		static string s_UIText;
		static string uiText
		{
			get
			{
				if (s_UIText == null)
					s_UIText = System.IO.File.ReadAllText(EditorApplication.applicationContentsPath + "/Resources/TreeEditor_UI_Strings_EN.txt");
		...
		internal void SetAnimsCallback(AnimValueManager.Callback callback)
		{
			if (m_Anims.callback == null)
				m_Anims.callback = callback;
		}
		...
		private bool GUITooManyShaders(NodeType nodeType)
		{
			...
			GUIContent message = GetGUIContent(nodeType == NodeType.BarkNode ?
				"TreeEditor.TreeGroup.TooManyBarkShaders" :
				"TreeEditor.TreeGroup.TooManyLeafShaders");
			GUIContent button = GetGUIContent("TreeEditor.TreeGroup.ChangeAllShadersOnNodesButton");
		...
		{
			GUIContent message = GetGUIContent("TreeEditor.TreeGroup.NotATreeShader");
			GUIContent button = GetGUIContent("TreeEditor.TreeGroup.ChangeShaderButton");
		...
		static public string GetUIString(string id)
		{
			string tempID = id + ":";
			int index = uiText.IndexOf(tempID);
			if (index < 0)
			{
				Debug.LogError("Missing definition for " + id + "!");
				return null;
			}
			int endIndex = uiText.IndexOf("\n", index);

			index += tempID.Length;
			return uiText.Substring(index, endIndex - index);
		}
