...
/*				MiniButton = "MiniButton";
				...
				MiniButtonMiddle = "MiniButtonMid";
...
*/				
		...
		private bool HasFlag(ConsoleFlags flags) { return (LogEntries.consoleFlags & (int)flags) != 0; }
		...
		static internal Texture2D GetIconForErrorMode (int mode, bool large)
		{
			...
			if (HasMode(mode, Mode.Fatal | Mode.Assert | 
				...
				Mode.AssetImportError | Mode.ScriptCompileError | Mode.GraphCompileError))
		...
		void OnGUI()
		{
			...
			try
			{
				...
				foreach (ListViewElement el in ListViewGUI.ListView (m_ListView, Constants.Box))
				{
					if (e.type == EventType.mouseDown && e.button == 0 && el.position.Contains(e.mousePosition))
					{
						if (e.clickCount == 2)
						{
							LogEntries.OpenEntryFile(m_ListView.row);
						}
			...
			EditorGUILayout.SelectableLabel (m_ActiveText, Constants.MessageStyle, GUILayout.ExpandWidth(true), GUILayout.ExpandHeight(true));
