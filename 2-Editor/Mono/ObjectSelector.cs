...
internal class ObjectSelector : EditorWindow
{
	...
	AnimValueManager m_AnimManager = new AnimValueManager ();
	...
	void OnEnable () 
	{
		m_AnimManager.callback = Repaint;
		m_AnimManager.speed = 1.5f;
		m_AnimManager.Add(m_ShowOverlapPreview);
		m_AnimManager.Add(m_ShowWidePreview);
	...
	void ItemSelectedCallback (bool doubleClicked)
	{
		if (doubleClicked)
		{
			Close();
	...
	static bool GuessIfUserIsLookingForAnAsset (string requiredClassName, bool checkGameObject)
	{
		string[] requiredClassNames =
		{
			...
			"Texture", "Cubemap", "MovieTexture", "RenderTexture", "Texture2D", "ProceduralTexture"
		};
	...
	void FilterSettingsChanged ()
	{
		...
		m_ListArea.Init (listPosition, m_IsShowingAssets ? HierarchyType.Assets : HierarchyType.GameObjects, filter, true);
	}

	// show the ObjectSelector
	public void Show (Object obj, System.Type requiredType, SerializedProperty property, bool allowSceneObjects)
	{
		...
		title = "Select " + requiredClassName;
		...
		else
		{
			...
			m_ListArea.InitSelection(new[] { instanceID });
	...
	void InitIfNeeded ()
	{
		...
		if (m_ListArea == null)
		{
			m_ListArea = new ObjectListArea(m_ListAreaState, this, true); //, false, false, false, false, false);
			...
			m_ListArea.itemSelectedCallback += ItemSelectedCallback;
			m_ListArea.gridSize = m_StartGridSize.value;

			SearchFilter filter = new SearchFilter();
			filter.nameFilter = m_SearchFilter;
			if (!string.IsNullOrEmpty(m_RequiredType))
				filter.classNames = new[] { m_RequiredType };
			m_ListArea.Init(listPosition, m_IsShowingAssets ? HierarchyType.Assets : HierarchyType.GameObjects, filter, true);
		}
	...
	void SearchArea()
	{
		...
		{
			m_ListArea.gridSize = (int)GUI.HorizontalSlider(new Rect(position.width - 60, 26, 55, m_ToolbarHeight - 28), m_ListArea.gridSize, m_ListArea.minGridSize, m_ListArea.maxGridSize);
		}
	}
	...
	void OnGUI ()
	{
		...
		GUI.BeginGroup (new Rect (0, 0, position.width, position.height), GUIContent.none); //, m_Styles.background);
		HandleKeyboard ();
