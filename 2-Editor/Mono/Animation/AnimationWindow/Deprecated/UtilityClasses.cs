...
// @TODO: there is no good reason for this cache being static. It should be instance variables...
internal class CurveRendererCache
{
	private static Hashtable m_CombiRenderers = new Hashtable(); // Maintain list of combined euler curve renderers
	private static Hashtable m_NormalRenderers = new Hashtable(); // Maintain list of combined euler curve renderers
	...
	public static CurveRenderer GetCurveRenderer (AnimationClip clip, EditorCurveBinding curveBinding)
	{
		...
		{
			int component = RotationCurveInterpolation.GetCurveIndexFromName(curveBinding.propertyName);
			
			string transformKey = CurveUtility.GetCurveGroupID (clip, curveBinding).ToString ();
			EulerCurveCombinedRenderer combiRenderer = (EulerCurveCombinedRenderer)m_CombiRenderers[transformKey];
			if (combiRenderer == null)
			{
				combiRenderer = new EulerCurveCombinedRenderer(
					AnimationUtility.GetEditorCurve (clip, EditorCurveBinding.FloatCurve (curveBinding.path, typeof (Transform), "m_LocalRotation.x")),
					AnimationUtility.GetEditorCurve (clip, EditorCurveBinding.FloatCurve (curveBinding.path, typeof (Transform), "m_LocalRotation.y")),
					AnimationUtility.GetEditorCurve (clip, EditorCurveBinding.FloatCurve (curveBinding.path, typeof (Transform), "m_LocalRotation.z")),
					AnimationUtility.GetEditorCurve (clip, EditorCurveBinding.FloatCurve (curveBinding.path, typeof (Transform), "m_LocalRotation.w")),
					AnimationUtility.GetEditorCurve (clip, EditorCurveBinding.FloatCurve (curveBinding.path, typeof (Transform), "localEulerAngles.x")),
					AnimationUtility.GetEditorCurve (clip, EditorCurveBinding.FloatCurve (curveBinding.path, typeof (Transform), "localEulerAngles.y")),
					AnimationUtility.GetEditorCurve (clip, EditorCurveBinding.FloatCurve (curveBinding.path, typeof (Transform), "localEulerAngles.z"))
					);
				m_CombiRenderers.Add(transformKey, combiRenderer);
			}
			
			return new EulerCurveRenderer(component, combiRenderer);
		}
		else
		{
			string transformKey = CurveUtility.GetCurveID (clip, curveBinding).ToString ();
			NormalCurveRenderer normalRenderer = (NormalCurveRenderer)m_NormalRenderers[transformKey];
			if (normalRenderer == null)
			{
				normalRenderer = new NormalCurveRenderer (AnimationUtility.GetEditorCurve (clip, curveBinding));
				m_NormalRenderers.Add(transformKey, normalRenderer);
			}
...
internal static class CurveUtility
{
	...
	public static int GetCurveID (AnimationClip clip, EditorCurveBinding curveData)
	{
		...
		return clipID * 19603 ^ curveData.path.GetHashCode() * 729 ^ curveData.type.GetHashCode() * 27 ^ curveData.propertyName.GetHashCode();
	}
	...
	public static int GetCurveGroupID (AnimationClip clip, EditorCurveBinding curveData)
	{
		...
		int clipID = ( clip == null ? 0 : clip.GetInstanceID() );
		string xName = curveData.propertyName.Substring(0,curveData.propertyName.Length-1);
		return clipID * 19603 ^ curveData.path.GetHashCode() * 729 ^ curveData.type.GetHashCode() *27 ^ xName.GetHashCode();
	}
	...
	static private float CalculateLinearTangent (AnimationCurve curve, int index, int toIndex)
	{
	...
	public static void SetKeyModeFromContext (AnimationCurve curve, int keyIndex)
	{
		...
		bool broken = false;
		if (keyIndex > 0 && GetKeyBroken(curve[keyIndex-1])) broken = true;
		if (keyIndex < curve.length-1 && GetKeyBroken(curve[keyIndex+1])) broken = true;
		
		SetKeyBroken(ref key, broken);
		
		if (broken)
		{
			if (keyIndex > 0) SetKeyTangentMode(ref key, 0, GetKeyTangentMode(curve[keyIndex-1], 1));
			if (keyIndex < curve.length-1) SetKeyTangentMode(ref key, 1, GetKeyTangentMode(curve[keyIndex+1], 0));
		}
		else
		{
			// If both neighbors are auto smooth, make this one auto smooth as well,
			// otherwise make it editable smooth
			TangentMode mode = TangentMode.Smooth;
			if (keyIndex > 0 && GetKeyTangentMode(curve[keyIndex-1], 1) != TangentMode.Smooth) mode = TangentMode.Editable;
			if (keyIndex < curve.length-1 && GetKeyTangentMode(curve[keyIndex+1], 0) != TangentMode.Smooth) mode = TangentMode.Editable;
	...
	public static Color GetPropertyColor (string name)
	{
		...
		{
			if      (name.EndsWith(".x")) col = AnimationWindow.kEulerXColor;
			else if (name.EndsWith(".y")) col = AnimationWindow.kEulerYColor;
			else if (name.EndsWith(".z")) col = AnimationWindow.kEulerZColor;
		}
		else if (type==3)
		{
		...
		else
		{
			...
			col = GetBalancedColor(EditorGUIUtility.HSVToRGB(rand, 1, 1));
		}
