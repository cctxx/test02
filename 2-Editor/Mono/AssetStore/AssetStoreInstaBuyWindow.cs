...
internal class AssetStoreInstaBuyWindow : EditorWindow {
	...
	static public AssetStoreInstaBuyWindow ShowAssetStoreInstaBuyWindow (AssetStoreAsset asset, string purchaseMessage, string paymentMethodCard, string paymentMethodExpire, string priceText) {
		...
		w.m_PriceText = priceText;
 		Analytics.Track(string.Format("/AssetStore/ShowInstaBuy/{0}/{1}", w.m_Asset.packageID, w.m_Asset.id) );
		return w;
	}
	...
	static public void ShowAssetStoreInstaBuyWindowBuilding (AssetStoreAsset asset) {
		...
		Analytics.Track(string.Format("/AssetStore/ShowInstaFree/{0}/{1}", w.m_Asset.packageID, w.m_Asset.id) );
	}
		...
		GUILayout.BeginHorizontal();
			...
			if (GUILayout.Button("Just put to basket...")) 
			{
				AssetStore.Open(string.Format("content/{0}/basketpurchase", m_Asset.packageID));																			
		 		Analytics.Track(string.Format("/AssetStore/PutToBasket/{0}/{1}", m_Asset.packageID, m_Asset.id) );
				m_Asset = null;
				this.Close();
				GUIUtility.ExitGUI ();				
			}
			...
			{
		 		Analytics.Track(string.Format("/AssetStore/CancelInstaBuy/{0}/{1}", m_Asset.packageID, m_Asset.id) );
				m_Asset = null;
				this.Close();
				GUIUtility.ExitGUI ();				
			}
	...
	void PurchaseSuccessGUI() 
	{
		...
		GUILayout.BeginHorizontal();
			...
			{
		 		Analytics.Track(string.Format("/AssetStore/PurchaseOk/{0}/{1}", m_Asset.packageID, m_Asset.id) );
				m_Asset = null;
				this.Close();
			}
			GUILayout.Space (5);
			...
			{
		 		Analytics.Track(string.Format("/AssetStore/PurchaseOkImport/{0}/{1}", m_Asset.packageID, m_Asset.id) );
				m_BuildAttempts = 1;
	...
	void PurchaseDeclinedGUI() 
	{
		...
		GUILayout.BeginHorizontal();
			...
			if (GUILayout.Button("Close")) 
			{
		 		Analytics.Track(string.Format("/AssetStore/DeclinedAbort/{0}/{1}", m_Asset.packageID, m_Asset.id) );
				m_Asset = null;
				Close();
			}
			GUILayout.Space (5);
			...
			{
				AssetStore.Open(string.Format("content/{0}/basketpurchase", m_Asset.packageID));
		 		Analytics.Track(string.Format("/AssetStore/DeclinedPutToBasket/{0}/{1}", m_Asset.packageID, m_Asset.id) );
				m_Asset = null;
				Close();
			}
			GUILayout.Space (5);
	...
	void CompletePurchase()
	{
		...
		Analytics.Track(string.Format("/AssetStore/InstaBuy/{0}/{1}", m_Asset.packageID, m_Asset.id) );	
	}
