...
//
	...
	public bool BeginRename (float delay)
	{
		...
		return GetRenameOverlay ().BeginRename (hierarchyProperty.name, instanceID, delay);
	}
			...
			if (IsLastClickedItemVisible ())
			{
				switch (Event.current.keyCode)
				{
					...
					case KeyCode.LeftArrow:
						if (!m_LocalAssets.ListMode && !IsPreviewIconExpansionModifierPressed ())
							offset = -1;
						break;
		...
		switch (m_SelectionOffset)
		{
			...
			case kPageDown:
				...
				else
				{
					...
					int remainingItems = maxIndex - idx - 1;
			...
			case kHome:
				...
				return;
			case kEnd:
				m_SelectionOffset = maxIndex - 1 - idx;
				break;
		}
	}
	...
	void Reveal (int instanceID)
	{
		...
		if (hierarchyProperty.Find (instanceID, null))
		{
			if (hierarchyProperty.Parent ())
			{
				if (!hierarchyProperty.isFolder)
				{
					// We only show one level of subassets so just expand parent asset
					int parentInstanceID = hierarchyProperty.instanceID;
					m_LocalAssets.ChangeExpandedState (parentInstanceID, true);
				}
			}
		}
	}
	...
	void SetupData (bool forceReflow)
	{
	...
	{
		return m_LocalAssets.m_Grid.GetMaxVisibleItems (m_TotalRect.height);
	}
