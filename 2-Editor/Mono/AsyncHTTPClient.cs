 ...
 * Http job
 * 
 * Jonas Drewsen - (C) Unity3d.com - 2010-2011
 * 
 * Takes care of a single http request to the Unity Asset Store.
 * This class is not used directly but through the Server class.
 */
using System;
using System.Net;
using System.IO;
...
internal partial class AsyncHTTPClient {
	...
	public delegate void StatusCallback(AsyncHTTPClient client);
	
	// public StatusCallback statusCallback; // TODO
...
//TODO: Implement PUT requests:
	/*
	 * PUT requests for uploading a file _fromPath.
	 */
/*	public AsyncHTTPClient(string _fromPath, string _toUrl)
	{
		m_ToUrl = _toUrl;
		m_FromData = _fromPath;
		m_Method = "PUT";
		state = State.INIT;
...
*/
	 ...
	 * to unique keys.
	 */
	public Dictionary<string,string> postDictionary { set {
    ...
    {
    	state=status;
    	//mLastMessage(string.Format("{0}/{1} bytes", bytesDone, bytesTotal));  	
