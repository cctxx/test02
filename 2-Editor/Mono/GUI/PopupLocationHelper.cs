...
internal static class PopupLocationHelper
{
	...
	public static Rect GetDropDownRect (Rect buttonRect, Vector2 minSize, Vector2 maxSize, ContainerWindow popupContainerWindow, PopupLocation[] locationPriorityOrder)
	{
		if (locationPriorityOrder == null)
			locationPriorityOrder = new [] {PopupLocation.Below, PopupLocation.Above}; // Default priority order
	...
	private static bool PopupRight (Rect buttonRect, Vector2 minSize, Vector2 maxSize, ContainerWindow popupContainerWindow, out Rect resultRect)
	{
		...
		dropDownRectRight = popupContainerWindow.FitWindowRectToScreen (dropDownRectRight, true, true);
	                               ...
	                               out Rect resultRect)
	{
		...
		dropDownRectLeft = popupContainerWindow.FitWindowRectToScreen (dropDownRectLeft, true, true);
	                                ...
	                                out Rect resultRect)
	{
		...
		// Fit rect to screen
		dropDownRectAbove = popupContainerWindow.FitWindowRectToScreen (dropDownRectAbove, true, true);
	                                ...
	                                out Rect resultRect)
	{
		...
		// Fit rect to screen
		dropDownRectBelow = popupContainerWindow.FitWindowRectToScreen (dropDownRectBelow, true, true);
	...
	private static float k_SpaceFromBottom
	{
		get
		{
			if (Application.platform == RuntimePlatform.OSXEditor)
				return 10f;
			return 70; // On Windows we dont know if the taskbar is showing so ensure proper space from bottom
		}
	}
}
