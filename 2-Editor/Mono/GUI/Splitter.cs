		...
		internal class GUISplitterGroup : GUILayoutGroup
		{
			...
			public override void SetVertical(float y, float height)
			{
				...
				if (isVertical)
				{
					...
					if (heightToDistribute > childMaxHeight) 
					{			// If we have too much space - stretch any stretchable children
				...
				case EventType.Repaint:
					{
						...
						for (var i = 0; i < state.relativeSizes.Length - 1; i++)
						{
							...
							EditorGUIUtility.AddCursorRect(splitterRect, g.isVertical ? MouseCursor.ResizeVertical : MouseCursor.SplitResizeLeftRight);
