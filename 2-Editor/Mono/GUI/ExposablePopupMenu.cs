	...
	public float OnGUI (Rect rect)
	{
		if (rect.width >= m_WidthOfButtons && rect.width > m_MinWidthOfPopup)
		{
			...
			foreach (var item in m_Items)
			{
				...
				EditorGUI.BeginDisabledGroup (!item.m_Enabled);
		...
		else 
		{
			...
			if (EditorGUI.ButtonMouseDown (rect, m_PopupButtonData.m_GUIContent, FocusType.Passive, m_PopupButtonData.m_Style))
