			...
			public bool MoveNext()
			{
						...
						GUIClip.visibleRect.Contains(Event.current.mousePosition))
					{
						...
						if (delay.CanStartDrag())
						{
							...
							if (ilvState.wantsReordering)
							{
								...
								if (ilvState.wantsToStartCustomDrag)
								{
				...
				if (quiting)
				{
					...
					if (ilvState.wantsReordering && (GUIUtility.hotControl == ilvState.state.ID))
					{
							...
							case EventType.DragPerform:
								{
									if (GUIClip.visibleRect.Contains(Event.current.mousePosition))
									{
										...
										// TODO: this doesn't work with multi selection
										//if (ilvState.state.draggedFrom == ilvState.state.draggedTo)
										//{
										//	ilvState.state.draggedFrom = -1;
										//	ilvState.state.draggedTo = -1;
										//}
										//else
										{
						...
						if (ilvState.wantsExternalFiles)
						{
							switch (Event.current.type)
							{
								case EventType.DragUpdated:
							...
							if (ilvState.wantsToAcceptCustomDrag && (dragControlID != ilvState.state.ID))
							{
								switch (Event.current.type)
								{
									case EventType.DragUpdated:
