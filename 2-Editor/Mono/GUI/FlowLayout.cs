...
namespace UnityEditor
{
	class FlowLayout : GUILayoutGroup 
	{
		struct LineInfo 
		{
			...
			public int topBorder, bottomBorder, expandSize;
		}
			...
			else
			{
				...
				if (!hasMinWidth) 
				{
					...
					foreach (GUILayoutEntry i in entries) {
						...
						minWidth = Mathf.Max (childMinWidth, i.minWidth); 
					}
				}
			}
		}
		...
		public override void CalcHeight ()
		{
			...
			else
			{
				...
				for (int i = 0; i < m_Lines; i++)
				{
					childMinHeight += m_LineInfo[i].minSize;
					childMaxHeight += m_LineInfo[i].maxSize;
				}
				...
				for (int i = 1; i < m_Lines; i++) 
				{
					...
					childMinHeight += space;
					childMaxHeight += space;
				}
			...
			{
				firstPadding = _topMarginMin;
				lastPadding = _bottomMarginMin;
			} 
			else*/
			{
...
/*		public Rect BeginVertical (int id, params GUILayoutOption[] options)
		{
			SlideGroupInternal g = (SlideGroupInternal)GUILayoutUtility.BeginLayoutGroup (GUIStyle.none, options, typeof (SlideGroupInternal));
			g.SetID (id);
			g.isVertical = true;
