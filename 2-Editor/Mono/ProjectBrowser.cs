...
internal class ProjectBrowser : EditorWindow, IHasCustomMenu
{
	...
	const float		k_BottomBarHeight = 17;
	...
	void OnEnable ()
	{
		title = "Project"; // The title is also used for fetching the project window tab icon: Project.png
	...
	void Awake ()
	{
		...
		Analytics.Event ("ProjectBrowser", "StartLayout", m_ViewMode.ToString(), 1);
		Analytics.Event ("ProjectBrowser", "WindowDimensions", "Width: " + GetAnalyticsSizeLabel (position.width) + ", Height: " + GetAnalyticsSizeLabel (position.height), 1);
		Analytics.Event ("ProjectBrowser", "WindowOrientation", (position.width > position.height) ? "Landscape" : "Portrait", 1);
		Analytics.Event ("ProjectBrowser", "GridSize", m_StartGridSize.ToString(), 1);
		Analytics.Event ("ProjectBrowser", "FolderGridSize", m_LastFoldersGridSize.ToString(), 1);
		Analytics.Event ("ProjectBrowser", "LocalAssetsSearchArea", m_LastLocalAssetsSearchArea.ToString (), 1);
	}
	...
	void EnsureValidFolders ()
	{
		...
		foreach (string folder in m_SearchFilter.folders)
		{
			...
			{
				// Find first valid parent folder
	...
	public void Init ()
	{
		...
		if (m_FolderTreeState == null)
			m_FolderTreeState= new TreeViewState ();
	...
	void SearchButtonClickedCallback (ExposablePopupMenu.ItemData itemClicked)
	{
		if (!itemClicked.m_On) // Behave like radio buttons: a button that is on cannot be turned off
		{
			...
			Analytics.Event ("ProjectBrowser", "UserChangedSearchArea", m_SearchFilter.searchArea.ToString (), 1);
		}
	}
	...
	void InitSearchMenu ()
	{
		...
		GUIStyle onStyle = "ExposablePopupItem";//EditorStyles.boldLabel;
		GUIStyle offStyle = "ExposablePopupItem";//EditorStyles.label;
		...
		switch (state)
		{
			case SearchViewState.AllAssets: popupButtonContent = m_SearchAllAssets;	break;
			case SearchViewState.SubFolders: popupButtonContent = m_SearchInFolders;	break;
			case SearchViewState.AssetStore: popupButtonContent = m_SearchAssetStore;	break;
			case SearchViewState.NotSearching: popupButtonContent = m_SearchAssetStore;	break;
	...
	void DefaultSetup ()
	{
		if (m_LastProjectPath != Directory.GetCurrentDirectory())
		{
			...
			// Default show the assets folder
	...
	{	
		...
		{
			m_AssetTree = new TreeView (this, m_AssetTreeState);
			...
			m_AssetTree.contextClickCallback += AssetTreeViewContextClick;
			...
			m_AssetTree.Init(	m_TreeViewRect,
								...
								new AssetOrGameObjectTreeViewDragging (m_AssetTree, HierarchyType.Assets)
								);
			m_AssetTree.ReloadData ();		
		}
		else if (m_ViewMode == ViewMode.TwoColumns)
		{
			m_FolderTree = new TreeView (this, m_FolderTreeState);
			m_FolderTree.deselectOnUnhandledMouseDown = false;
			m_FolderTree.selectionChangedCallback += FolderTreeSelectionCallback;
			m_FolderTree.contextClickCallback += FolderTreeViewContextClick;
	...
	public void TypeListCallback(PopupList.ListElement element)
	{
		...
		for (int i=0; i<selectedDisplayNames.Length; ++i)
			selectedDisplayNames[i] = selectedDisplayNames[i];
	...
	public static bool IsFolder (int instanceID)
	{
		...
		if (hierarchyProperty.Find (instanceID, null))
			return hierarchyProperty.isFolder;	
		return false;
	}
		...
		if (Event.current.type == EventType.keyDown)
		{
			switch (Event.current.keyCode)
			{
				...
				case KeyCode.Backspace:
					if (Application.platform == RuntimePlatform.WindowsEditor && m_ViewMode == ViewMode.TwoColumns)
					{
						ShowParentFolderOfCurrentlySelected ();
						Event.current.Use(); 
					}
	...
	void ShowParentFolderOfCurrentlySelected ()
	{
		if (IsShowingFolderContents ())
		{
			...
			{
				TreeViewItem item = m_FolderTree.FindNode (selectedFolderInstanceIDs[0]);
	...
	int GetParentFolderInstanceID (int instanceID)
	{
	...
	void OnSelectionChange () 
	{
		...
		if (m_ViewMode == ViewMode.OneColumn)
		{
			...
			bool revealSelectionAndFrameLast = !m_IsLocked;
		...
		else if (m_ViewMode == ViewMode.TwoColumns)
		{
			...
			{
				bool frame = !m_IsLocked && Selection.instanceIDs.Length > 0;
				if (frame)
				{
					...
					else
					{
						if (frame)
						{
							FrameObjectPrivate(instanceID, true, false);
						}
					}
				}
			}
	...
	void SetSearchFoldersFromCurrentSelection ()
	{
		...
		foreach (int instanceID in Selection.instanceIDs)
		{
			...
			if (search.Find (instanceID, null))
			{
				int i=0;
				while (search.isValid && !search.isFolder && i++ < 3)
					search.Parent ();

				if (search.isValid && search.isFolder)
					folders.Add (AssetDatabase.GetAssetPath (search.instanceID));
			}
		}
		...
		if (eventType == EventType.ExecuteCommand || eventType == EventType.ValidateCommand)
		{
			...
			if (Event.current.commandName == "Delete" || Event.current.commandName == "SoftDelete")
			{
				...
				if (execute)
				{
					...
					{
						Assert.That (instanceIDs.Length == 1); //We do not support multiselection for filters
	...
	void SelectAll ()
	{
		if (m_ViewMode == ViewMode.OneColumn)
		{
			...
			{
				int[] instanceIDs = m_AssetTree.GetVisibleRowIDs ();
	...
	{
		return m_SearchFilter.GetState () == SearchFilter.State.EmptySearchFilter ? 0f : 18f;
	}
	...
	void OnGUI ()
	{
		...
		m_ToolbarHeight = EditorStyles.toolbar.fixedHeight;
		...
		if (m_ViewMode == ViewMode.OneColumn)
		{
			...
			else
			{
				...
				HandleUnusedAssetDragEvents (m_TreeViewRect);
			}
		}
		else // ViewMode.TwoColumns
		{
			...
			if (evt.type == EventType.repaint)
			{
				...
				GUI.color = GUI.color * ((EditorGUIUtility.isProSkin) ? new Color(0.12f, 0.12f, 0.12f, 1.333f) : new Color(0.6f, 0.6f, 0.6f, 1.333f)); // dark : light
				Rect splitterRect = new Rect (m_ListAreaRect.x-1, m_ToolbarHeight, 1, m_TreeViewRect.height);
	...
	static void HandleUnusedAssetDragEvents (Rect rect)
	{
		Event evt = Event.current;
		switch (evt.type)
		{
			...
			case EventType.DragPerform:
				...
				{
					int instanceID = ProjectBrowserColumnOneTreeViewDataSource.GetAssetsFolderInstanceID ();
					...
					DragAndDrop.visualMode = InternalEditorUtility.ProjectWindowDrag (search, perform);
					evt.Use ();
				}
				break;
		}
	...
	void AssetTreeViewContextClick (int clickedNodeInstanceID)
	{
		Event evt = Event.current;
		Assert.That (evt.type == EventType.ContextClick);

		if (clickedNodeInstanceID == 0)
		{
			// Deselect all
	...
	void FolderTreeViewContextClick (int clickedNodeInstanceID)
	{
		Event evt = Event.current;
		Assert.That(evt.type == EventType.ContextClick);
		if (SavedSearchFilters.IsSavedFilter(clickedNodeInstanceID))
		{
			...
			SavedFiltersContextMenu.Show(clickedNodeInstanceID);
		}
		else
		{
			// Context click in deadspace (clickedNodeInstanceID == 0) is ambigous due to Favorites and Assets root items so menu is not shown here.
			if (clickedNodeInstanceID != 0)
			{
				EditorUtility.DisplayPopupMenu(new Rect(evt.mousePosition.x, evt.mousePosition.y, 0, 0), "Assets/", null);
			}
	...
	public virtual void AddItemsToMenu (GenericMenu menu)
	{
		if (m_EnableOldAssetTree)
		{
			...
			GUIContent assetBrowserText =	new GUIContent ("Two Columns Layout");
			...
			if (Unsupported.IsDeveloperBuild())
				menu.AddItem (new GUIContent ("Debug TreeView"), false, OpenTreeViewTestWindow);
		}
	}
	...
	void ResizeHandling (float width, float height)
	{
		...
		// For horizontal layout we add a vertical size controller to adjust dir vs icon area
		...
		float newDirectoryWidth = 0;

		// Drag internal splitter
		float deltaX = EditorGUI.MouseDeltaReader(dragRect, true).x;
		if (deltaX != 0f)
		{
			m_DirectoriesAreaWidth += deltaX;
			float maxWidth = width - k_MinDirectoriesAreaWidth;
			newDirectoryWidth = Mathf.Clamp (m_DirectoriesAreaWidth, k_MinDirectoriesAreaWidth, maxWidth);
		}

		// When dragging window borders we need to ensure best fit
		float listAreaMinWidth = k_MinWidthTwoColumns - k_MinDirectoriesAreaWidth;
		if (width - m_DirectoriesAreaWidth < listAreaMinWidth)
			newDirectoryWidth = width - listAreaMinWidth;

		if (newDirectoryWidth > 0)
		{
			m_DirectoriesAreaWidth = newDirectoryWidth;
		}
	...
	void ButtonSaveFilter ()
	{
		...
		EditorGUI.BeginDisabledGroup (!m_SearchFilter.IsSearching ());
		{
	...
	void CreateDropdown ()
	{
		...
		if (EditorGUI.ButtonMouseDown (r, s_Styles.m_CreateDropdownContent, FocusType.Passive, EditorStyles.toolbarDropDown))
		{
			GUIUtility.hotControl = 0;
	...
	void AssetLabelsDropDown ()
	{
		...
		if (EditorGUI.ButtonMouseDown(r, s_Styles.m_FilterByLabel, FocusType.Passive, EditorStyles.toolbarButton))
		{
			//Rect rect = GUILayoutUtility.topLevel.GetLast();
			if (PopupList.ShowAtPosition(new Vector2(r.x, r.y + r.height), m_AssetLabels, PopupList.Gravity.Top ))
			{
				GUIUtility.ExitGUI();
			}
		}		
	...
	void TypeDropDown ()
	{
		...
		if (EditorGUI.ButtonMouseDown(r, s_Styles.m_FilterByType, FocusType.Passive, EditorStyles.toolbarButton))
		{
			Rect rect = GUILayoutUtility.topLevel.GetLast();
			if (PopupList.ShowAtPosition(new Vector2(rect.x, rect.y + rect.height), m_ObjectTypes, PopupList.Gravity.Top ))
			{
				GUIUtility.ExitGUI();
			}
		}		
...
/*		
	...
	{
		Debug.Log ("OnChanged File: " + e.FullPath + " " + e.ChangeType);
	}

	private static void OnRenamed (object source, RenamedEventArgs e)
	{
		Debug.Log ("OnRenamed File: " + e.OldFullPath + " new: " + e.FullPath);
	}
		*/				
	...
	void BreadCrumbBar ()
	{
		...
		if (m_BreadCrumbs.Count == 0)
		{
			...
			m_BreadCrumbLastFolderHasSubFolders = BreadCrumbListMenu.GetSubFolders (path).Count > 0; 
		}
		...
		if (m_SearchFilter.folders.Length == 1)
		{
			for (int i=0; i < m_BreadCrumbs.Count; ++i)
			{
				...
				if (!lastElement || m_BreadCrumbLastFolderHasSubFolders)
				{
					...
					if (EditorGUI.ButtonMouseDown (buttonRect, GUIContent.none, FocusType.Passive, s_Styles.foldout))
					{
	...
	internal void BeginPreimportedNameEditing (int instanceID, EndNameEditAction endAction, string pathName, Texture2D icon, string resourceFile)
	{
		...
		if (m_ViewMode == ViewMode.TwoColumns)
		{
			...
			m_ListAreaState.m_CreateAssetUtility.BeginNewAssetCreation (instanceID, endAction, pathName, icon, resourceFile);
	...
	{
		FrameObjectPrivate (instanceID, !m_IsLocked, ping);
		...
		if (m_ViewMode == ViewMode.TwoColumns)
		{
			...
			if (hierarchyProperty.Find (instanceID, null))
			{
				while (hierarchyProperty.Parent ())
				{
					if (hierarchyProperty.isFolder)
					{
						folderInstanceID = hierarchyProperty.instanceID; 
						break;
					}
				}
