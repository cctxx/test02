...
public partial class Highlighter
{
	...
	private static GUIStyle highlightStyle { get {
	...
	private static bool SetWindow (string windowTitle)
	{
		...
		foreach (GUIView currentView in views)
		{
			...
			{
				if (EditorGUIUtility.TextContent ((currentView as HostView).actualView.title).text == windowTitle)
				{
					view = currentView;
					break;
				}
	...
	internal static void ControlHighlightGUI (GUIView self)
	{
		...
		// Set pulsing values
		GUI.color = new Color (1,1,1,0.8f - 0.3f * pulseValue);
