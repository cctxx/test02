...
)
{
	for (int t = 0; t < triCount; ++t, indices += 3)
	{
		...
		Point2D v0, v1, v2;
		v0.x = vertices[idx0].uv.x * width;
		v0.y = vertices[idx0].uv.y * height;
		v1.x = vertices[idx1].uv.x * width;
		v1.y = vertices[idx1].uv.y * height;
		v2.x = vertices[idx2].uv.x * width;
		v2.y = vertices[idx2].uv.y * height;
